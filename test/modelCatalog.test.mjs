...
test('별도 비교 수치가 없는 항목은 운영 기능으로 표시한다', () => {
  ...
  for (const entry of pendingEntries) {
    ...
    assert.match(entry.evidence.label.ko, /운영 기능/)
    assert.match(entry.evidence.label.en, /Production capability/)
  }
})

test('감성 seed 17 개발셋 결과를 출처별로 표시한다', () => {
  ...
  assert.deepEqual(news.metrics?.map((metric) => metric.display.ko), ['0.6183', '0.5707', '+8.34% (+4.76점)'])
  assert.deepEqual(disclosure.metrics?.map((metric) => metric.display.ko), ['0.9252', '0.9135', '+1.27% (+1.17점)'])
  assert.match(news.evidence.note.ko, /seed 17/)
  assert.match(disclosure.evidence.note.ko, /KR-FinBERT-SC/)
})
...
test('시장영향은 KR-FinBERT-SC 동일 조건 결과와 검정 상태를 출처별로 표시한다', () => {
  ...
  assert.match(disclosure.evidence.label.ko, /동일 조건 비교/)
})
