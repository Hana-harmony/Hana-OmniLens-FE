...
function PasswordPage({ session, onChanged, onHome, onAdmin, onPortal, onSignOut }: { session: Session | null; onChanged: (session: Session) => void; onHome: () => void; onAdmin: () => void; onPortal: () => void; onSignOut: () => void }) {
  ...
  const navItems: ConsoleNavItem<'dashboard' | 'api-keys' | 'analytics' | 'tax'>[] = admin
  ...
  return <ConsoleShell title={admin ? 'Hana OmniLens 관리자 백오피스' : '파트너 포털'} user={session.user} navItems={navItems} activeNav="password" onNavigate={() => admin ? onAdmin() : onPortal()} onHome={onHome} onPassword={() => undefined} onSignOut={onSignOut}><div className="password-page embedded">{form}</div></ConsoleShell>
}
