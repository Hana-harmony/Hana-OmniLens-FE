...
function App() {
  ...
  if (route === 'password' || session?.passwordChangeRequired) return <PasswordPage session={session} onChanged={passwordChanged} onHome={() => navigate('home')} onAdmin={() => navigate('admin')} onPortal={() => navigate('portal')} onSignOut={signOut} />
  if (route === 'docs') return <DocsPage locale={locale} onLocale={changeLocale} onHome={() => navigate('home')} />
  if (route === 'admin') return <AdminPage session={session} onSignIn={() => navigate('auth')} onHome={() => navigate('home')} onPassword={() => navigate('password')} onSignOut={signOut} />
  if (route === 'portal') return <MemberPortal session={session} onSignIn={() => navigate('auth')} onHome={() => navigate('home')} onAdmin={() => navigate('admin')} onPassword={() => navigate('password')} onSignOut={signOut} />
...
function PasswordPage({ session, onChanged, onHome, onAdmin, onPortal, onSignOut }: { session: Session | null; onChanged: (session: Session) => void; onHome: () => void; onAdmin: () => void; onPortal: () => void; onSignOut: () => void }) {
  ...
  const form = <form onSubmit={submit}><p className="eyebrow">ACCOUNT SECURITY</p><h1>{session.passwordChangeRequired ? '최초 비밀번호 변경' : '비밀번호 변경'}</h1><p>{session.passwordChangeRequired ? '초기 비밀번호를 변경해야 관리자 기능을 사용할 수 있습니다.' : '현재 비밀번호를 확인한 뒤 새 비밀번호를 설정합니다.'}</p><label>현재 비밀번호<input name="currentPassword" type="password" autoComplete="current-password" required/></label><label>새 비밀번호<input name="newPassword" type="password" minLength={8} maxLength={128} autoComplete="new-password" required/></label><label>새 비밀번호 확인<input name="newPasswordConfirmation" type="password" minLength={8} maxLength={128} autoComplete="new-password" required/></label>{message && <p className="form-error">{message}</p>}<button className="primary full" disabled={loading}>{loading ? '변경 중...' : '비밀번호 변경'}</button></form>
  if (session.passwordChangeRequired) return <div className="access-gate password-page"><Wordmark onClick={onHome}/>{form}</div>
  const admin = session.user.role === 'ADMIN'
  const navItems: ConsoleNavItem<'dashboard' | 'api-keys' | 'analytics' | 'tax' | 'password'>[] = admin
    ? [{ id: 'dashboard', label: '대시보드' }, { id: 'api-keys', label: 'API 키 관리' }, { id: 'analytics', label: '사용량 분석' }, { id: 'tax', label: '제한세율 적용신청 처리' }, { id: 'password', label: '비밀번호 변경' }]
    : [{ id: 'dashboard', label: '대시보드' }, { id: 'api-keys', label: 'API 키 관리' }, { id: 'password', label: '비밀번호 변경' }]
  return <ConsoleShell title={admin ? 'Hana OmniLens 관리자 백오피스' : '파트너 포털'} user={session.user} navItems={navItems} activeNav="password" onNavigate={(tab) => tab !== 'password' && (admin ? onAdmin() : onPortal())} onHome={onHome} onPassword={() => undefined} onSignOut={onSignOut}><div className="password-page embedded">{form}</div></ConsoleShell>
}

function MemberPortal({ session, onSignIn, onHome, onAdmin, onPassword, onSignOut }: { session: Session | null; onSignIn: () => void; onHome: () => void; onAdmin: () => void; onPassword: () => void; onSignOut: () => void }) {
  const [applications, setApplications] = useState<Application[]>([]); const [message, setMessage] = useState(''); const [activeTab, setActiveTab] = useState<'dashboard' | 'api-keys'>('dashboard')
  ...
  const navItems: ConsoleNavItem<'dashboard' | 'api-keys'>[] = [{ id: 'dashboard', label: '대시보드' }, { id: 'api-keys', label: 'API 키 관리' }]
  return <ConsoleShell title="파트너 포털" user={session.user} navItems={navItems} activeNav={activeTab} onNavigate={setActiveTab} onHome={onHome} onPassword={onPassword} onSignOut={onSignOut} adminAction={session.user.role === 'ADMIN' ? onAdmin : undefined}>{activeTab === 'dashboard' ? <><div className="console-hero"><div><p className="eyebrow">PARTNER WORKSPACE</p><h2>{session.user.name}님, 반갑습니다.</h2><p>발급된 API 키와 신청 상태를 안전하게 관리하세요.</p></div><button className="primary" onClick={() => setActiveTab('api-keys')}>API 키 관리 →</button></div>{message && <p className="notice">{message}</p>}<div className="admin-summary member-summary"><Summary label="전체 신청" value={applications.length}/><Summary label="승인된 API 키" value={applications.filter((item) => item.status === 'APPROVED').length}/></div></> : <section className="admin-panel tab-panel"><div className="panel-title"><div><p className="eyebrow">API KEY MANAGEMENT</p><h1>API 키 관리</h1><p>신청 상태와 발급된 키를 확인합니다.</p></div><button className="primary" onClick={requestKey}>API 키 신청</button></div>{message && <p className="notice">{message}</p>}<ApplicationTable applications={applications} onAction={action}/></section>}</ConsoleShell>
}

function AdminPage({ session, onSignIn, onHome, onPassword, onSignOut }: { session: Session | null; onSignIn: () => void; onHome: () => void; onPassword: () => void; onSignOut: () => void }) {
  const [applications, setApplications] = useState<Application[]>([]); const [analytics, setAnalytics] = useState<TermAnalytics>({ period: 'DAY', points: [], terms: [] }); const [cases, setCases] = useState<TaxCase[]>([]); const [message, setMessage] = useState(''); const [activeTab, setActiveTab] = useState<'dashboard' | 'api-keys' | 'analytics' | 'tax'>('dashboard')
...
function TaxAdmin({ cases, token, onChanged }: { cases: TaxCase[]; token: string; onChanged: () => Promise<void> }) {
  ...
  const labels: Record<string, string> = { receiptNumber: '접수번호', receiptDate: '접수일자', claimantName: '청구인 성명', birthDate: '생년월일', taxpayerIdentificationNumber: '납세자번호', claimantPhone: '전화번호', claimantResidence: '거주지국', residencyCountryCode: '거주지국 코드', claimantAddress: '주소', agentName: '대리인 성명', agentPhone: '대리인 전화번호', agentAddress: '대리인 주소', withholdingAgentName: '원천징수의무자 성명', withholdingAgentTaxId: '원천징수의무자 납세자번호', taxOffice: '세무서', withholdingAgentPhone: '원천징수의무자 전화번호', withholdingAgentAddress: '원천징수의무자 주소', claimContent: '청구 내용', applicationDate: '신청일', claimantSignatureName: '청구인 서명 성명', agentSignatureName: '대리인 서명 성명' }
  return <section className="admin-panel"><p className="eyebrow">TAX OPERATIONS</p><h2>제한세율 적용신청 관리</h2>{cases.length === 0 ? <p className="empty">동기화된 신청 건이 없습니다.</p> : cases.map((item) => <button className="case-row" key={item.caseId} onClick={() => void open(item)}><div><b>{item.caseId}</b><span>{item.taxYear} · {item.treatyCountry}</span></div><strong>{item.status === 'REFUND_APPROVED' ? '승인' : item.correctionRequestStatus === 'PREPARED' ? '작성 완료' : '미작성'}</strong></button>)}{selected && <div className="tax-editor"><div className="editor-heading"><div><p className="eyebrow">경정청구서 웹 편집기</p><h3>{selected.caseId}</h3></div><button className="secondary" onClick={() => void autoFill()}>정보 자동 불러오기</button></div><div className="document-previews">{selected.verifiedDocuments.map((document) => <SecureDocumentPreview key={document.documentId} document={document} caseId={selected.caseId} token={token}/>)}</div><div className="correction-form">{Object.entries(labels).map(([key, label]) => <label key={key}>{label}{key === 'claimContent' ? <textarea rows={4} value={fields[key] ?? ''} onChange={(event) => setFields({ ...fields, [key]: event.target.value })}/> : <input value={fields[key] ?? ''} onChange={(event) => setFields({ ...fields, [key]: event.target.value })}/>}</label>)}</div><div className="editor-actions"><button onClick={() => void save()}>저장하기</button><button onClick={() => void download()}>PDF 다운로드</button><button className="primary" disabled={selected.status === 'REFUND_APPROVED'} onClick={() => void submit()}>승인 및 국세청 제출</button></div></div>}{message && <p className="notice">{message}</p>}</section>
}
function SecureDocumentPreview({ document, caseId, token }: { document: VerifiedDocument; caseId: string; token: string }) {
  const [url, setUrl] = useState('')
  useEffect(() => { let active = true; let objectUrl = ''; void fetch(`${apiBaseUrl}/api/v1/portal/admin/tax/refund-cases/${caseId}/documents/${document.documentId}`, { headers: { Authorization: `Bearer ${token}` } }).then(async (response) => { if (!response.ok) throw new Error('문서를 불러오지 못했습니다.'); objectUrl = URL.createObjectURL(await response.blob()); if (active) setUrl(objectUrl) }).catch(() => active && setUrl('')); return () => { active = false; if (objectUrl) URL.revokeObjectURL(objectUrl) } }, [caseId, document.documentId, token])
  const pdf = document.fileName.toLowerCase().endsWith('.pdf')
  return <article><b>{documentTypeLabel(document.documentType)}</b><span>{document.fileName}</span>{url ? pdf ? <iframe title={document.fileName} src={url}/> : <img src={url} alt={document.fileName}/> : <div className="document-loading">불러오는 중…</div>}</article>
}
