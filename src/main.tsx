...
function ProductStory({ locale, title, intro }: { locale: Locale; title: string; intro: string }) {
  ...
  const stories = locale === 'ko' ? [
    ...
    { tag: '핵심 기능 03', title: '글로벌 세무 처리 자동화', body: '거주자 증명서, 아포스티유, 제한세율 적용신청서를 OCR로 읽고 필수값·일관성·위변조 위험을 검증합니다.', detail: '세 문서가 모두 검증된 경우에만 환급 신청 상태가 다음 단계로 진행됩니다.', image: '/showcase/exchange-tax-phone.png', focus: 'focus-tax' },
  ] : [
    ...
    { tag: 'CORE 03', title: 'Global tax automation', body: 'OCR validation for residence certificates, apostilles, and reduced withholding applications.', detail: 'The refund workflow advances only after all three documents pass validation.', image: '/showcase/exchange-tax-phone.png', focus: 'focus-tax' },
  ]
