...
const copy = {
  ko: {
    ...
    ai: 'Hana Montana AI', aiTitle: MODEL_OVERVIEW.title, aiBody: MODEL_OVERVIEW.body.ko,
  },
  en: {
    ...
    ai: 'Hana Montana AI', aiTitle: MODEL_OVERVIEW.title, aiBody: MODEL_OVERVIEW.body.en,
  },
} as const
...
function Home({ locale, onLocale, navigate, session, onSignOut }: { locale: Locale; onLocale: (locale: Locale) => void; navigate: (route: Route) => void; session: Session | null; onSignOut: () => void }) {
  ...
  return <div className="site-shell"><Header locale={locale} onLocale={onLocale} navigate={navigate} session={session} onSignOut={onSignOut}/><main>
    ...
    <section className="cta"><Wordmark onClick={() => navigate('home')} inverse/><div className="cta-copy"><p className="eyebrow">BUILD WITH HANA</p><h2>{locale === 'ko' ? '한국 금융 인텔리전스를 서비스에 연결하세요.' : 'Connect Korean financial intelligence to your product.'}</h2></div><button className="primary light" onClick={() => navigate('auth')}>{t.start} →</button></section>
...
function ModelPerformance({ locale }: { locale: Locale }) {
  ...
  type ComparisonGroup = { title: string; subtitle: string; baselineName: string; rows: ModelComparison[] }
  ...
  const comparisons: ComparisonGroup[] = locale === 'ko' ? [
    {
      ...
      rows: [
        { scope: '국내 뉴스', hana: 0.6183, baseline: 0.5707, improvement: '+8.34% (+4.76점)' },
        { scope: '국내 공시', hana: 0.9252, baseline: 0.9135, improvement: '+1.27% (+1.17점)' },
      ],
    },
    {
      title: '시장영향 중요도',
      ...
      baselineName: 'KR-FinBERT-SC',
      rows: [
        { scope: '국내 뉴스', hana: 0.3745, baseline: 0.3506, improvement: '+6.82% (+2.39점)' },
        { scope: '국내 공시', hana: 0.3216, baseline: 0.3131, improvement: '+2.72% (+0.85점)' },
      ],
    },
  ] : [
    {
      ...
      baselineName: 'KR-FinBERT-SC',
      rows: [
        { scope: 'Korean news', hana: 0.6183, baseline: 0.5707, improvement: '+8.34% (+4.76 pts)' },
        { scope: 'Korean disclosures', hana: 0.9252, baseline: 0.9135, improvement: '+1.27% (+1.17 pts)' },
      ],
    },
    {
      title: 'Market-impact importance',
      subtitle: 'Macro-F1 · fixed temporal Test · four classes',
      baselineName: 'KR-FinBERT-SC',
      rows: [
        { scope: 'Korean news', hana: 0.3745, baseline: 0.3506, improvement: '+6.82% (+2.39 pts)' },
        { scope: 'Korean disclosures', hana: 0.3216, baseline: 0.3131, improvement: '+2.72% (+0.85 pts)' },
      ],
    },
  ]
  ...
  return <section className="performance-section">
    ...
    <article className="research-spotlight">
      <div className="research-copy"><p className="eyebrow">K-FNSPID V4 RESEARCH</p><h3>{locale === 'ko' ? '하모니팀이 만든 한국 금융 빅데이터와 금융 특화 AI' : 'Korean financial big data and AI built by Team Harmony'}</h3><p>{locale === 'ko' ? 'K-FNSPID는 하모니팀이 한국 금융 뉴스와 전자공시를 대상 종목·한국 거래일·일별 시세에 직접 연결하고, 시간 누수와 중복 사건을 통제해 구축한 한국 금융 빅데이터입니다. 하모니팀은 K-FNSPID를 기반으로 금융 AI를 연구해 논문을 작성 중이며, 이 데이터로 학습한 Hana Montana AI는 동일 조건 비교에서 대상 종목 감성과 시장영향 중요도의 국내 뉴스·공시 점수가 모두 KR-FinBERT-SC보다 높았습니다.' : 'K-FNSPID is Korean financial big data built directly by Team Harmony by linking financial news and disclosures to target securities, Korean trading days, and daily prices while controlling temporal leakage and duplicate events. Team Harmony is researching financial AI with K-FNSPID and is currently writing a paper. Hana Montana AI, trained on this data, outperformed KR-FinBERT-SC across Korean news and disclosures for both target-aware sentiment and market-impact importance under the displayed same-condition comparisons.'}</p><dl><div><dt>1,247,685</dt><dd>{locale === 'ko' ? '뉴스·공시 문서' : 'news and disclosure documents'}</dd></div><div><dt>1,136,118</dt><dd>{locale === 'ko' ? '문서–종목 관계' : 'document-security relations'}</dd></div><div><dt>10,691,998</dt><dd>{locale === 'ko' ? '파일 기반 일별 시세 행' : 'file-based daily-price rows'}</dd></div></dl></div>
