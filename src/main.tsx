...
function ModelPerformance({ locale }: { locale: Locale }) {
  ...
  const features = locale === 'ko' ? [
    ...
    { title: '외국인 보유수량 예측', model: 'Stock-routed Panel Time-series ML Ensemble', description: '외국인 보유 제한 32개 종목의 일별 이력으로 장중 보유수량과 지분율을 예측합니다.', benchmarks: foreignBenchmarks },
    ...
    { title: 'Foreign ownership forecast', model: 'Stock-routed Panel Time-series ML Ensemble', description: 'Daily history predicts intraday ownership quantity and ratio for 32 foreign-limit stocks.', benchmarks: foreignBenchmarks },
...
function Footer({ locale }: { locale: Locale }) { return <footer><div className="footer-brands"><img src="/brand/hana-financial-group-ko-en.svg" alt="하나금융그룹 Hana Financial Group"/><img src="/brand/hana-securities-ko-en.svg" alt="하나증권 Hana Securities"/></div><p>{locale === 'ko' ? '해외 파트너를 위한 한국 금융 인텔리전스 API' : 'Korean financial intelligence for global partners'}</p><span>© 2026 하나 청년 금융인재<br/>양성 프로젝트 하모니팀</span></footer> }
