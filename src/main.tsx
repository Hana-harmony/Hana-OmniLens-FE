...
const copy = {
  ko: {
    ...
    ai: 'Hana Montana AI', aiTitle: 'Hana Montana AI(KF-DeBERTa + K-FNSPID)', aiBody: 'KF-DeBERTa 금융 감성·공시 의미 중요도와 K-FNSPID v4 뉴스·공시 출처별 시장영향 전문가를 분리해 제공하는 전용 금융 AI입니다. 공개 Test와 운영 Gold를 분리하고, 출처별 품질 gate를 통과한 모델만 요청 출처에 맞춰 실행합니다.',
  },
  en: {
    ...
    ai: 'Hana Montana AI', aiTitle: 'Hana Montana AI(KF-DeBERTa + K-FNSPID)', aiBody: 'A dedicated financial AI that separates KF-DeBERTa sentiment and semantic disclosure materiality from K-FNSPID v4 source-routed news and disclosure market-impact experts. Public Test and operational Gold sets remain separate, and only source-matched models that pass their quality gates are served.',
  },
} as const
...
function ModelPerformance({ locale }: { locale: Locale }) {
  ...
  const features = locale === 'ko' ? [
    ...
    { title: '금융 뉴스 감성', model: 'Hana Montana AI(KF-DeBERTa + K-FNSPID) · Sentiment Ensemble', description: 'KF-DeBERTa LoRA 80%와 누출 없는 기준 모델 20%의 확률 앙상블입니다. 공개 금융 Test와 실제 한국 뉴스 Gold를 분리해 일반화와 운영 품질을 각각 검증합니다.', metrics: [{ label: '공개 감성 Test macro F1', value: 88.4, display: '0.8840' }, { label: '실제 뉴스 Gold 정확도', value: 90, display: '90.00%' }] },
    ...
    { title: 'Financial-news sentiment', model: 'Hana Montana AI(KF-DeBERTa + K-FNSPID) · Sentiment Ensemble', description: 'A probability ensemble combines KF-DeBERTa LoRA at 80% with a leakage-safe baseline at 20%. A public financial Test set and real Korean-news Gold set measure generalization and operational quality separately.', metrics: [{ label: 'Public sentiment Test macro F1', value: 88.4, display: '0.8840' }, { label: 'Real-news Gold accuracy', value: 90, display: '90.00%' }] },
