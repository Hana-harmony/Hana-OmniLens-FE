...
type TaxCase = { caseId: string; accountId: string; taxYear: number; treatyCountry: string; estimatedRefundUsd: string; status: string; taxOfficeSubmissionStatus: string; correctionRequestStatus: string; verifiedDocuments: VerifiedDocument[] }
...
function App() {
  ...
  if (route === 'password' || session?.passwordChangeRequired) return <PasswordPage session={session} onChanged={passwordChanged} onHome={() => navigate('home')} />
...
function AuthPage({ locale, onLocale, onAuthenticated, onHome }: { locale: Locale; onLocale: (locale: Locale) => void; onAuthenticated: (session: Session) => void; onHome: () => void }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setLoading(true); setMessage(''); const form = new FormData(event.currentTarget); const password = String(form.get('password') ?? ''); const passwordConfirmation = String(form.get('passwordConfirmation') ?? ''); if (mode === 'signup' && password !== passwordConfirmation) { setMessage('비밀번호 확인이 일치하지 않습니다.'); setLoading(false); return } try { const body = mode === 'login' ? { username: form.get('username'), password } : { username: form.get('username'), password, passwordConfirmation, name: form.get('name'), phoneNumber: form.get('phoneNumber') }; onAuthenticated(await api<Session>(`/api/v1/portal/auth/${mode === 'login' ? 'login' : 'sign-up'}`, { method: 'POST', body: JSON.stringify(body) })) } catch (error) { setMessage(error instanceof Error ? error.message : 'Authentication failed') } finally { setLoading(false) } }
...
function PasswordPage({ session, onChanged, onHome }: { session: Session | null; onChanged: (session: Session) => void; onHome: () => void }) {
...
function AdminPage({ session, onSignIn, onHome, onPassword, onSignOut }: { session: Session | null; onSignIn: () => void; onHome: () => void; onPassword: () => void; onSignOut: () => void }) {
  const [applications, setApplications] = useState<Application[]>([]); const [stats, setStats] = useState<TermStat[]>([]); const [cases, setCases] = useState<TaxCase[]>([]); const [message, setMessage] = useState(''); const [activeTab, setActiveTab] = useState<'dashboard' | 'api-keys' | 'analytics' | 'tax'>('dashboard')
  ...
  const approve = async (id: string) => { try { await api(`/api/v1/portal/admin/api-key-applications/${id}/approve`, { method: 'POST' }, session.accessToken); await refresh() } catch (error) { setMessage(error instanceof Error ? error.message : '승인 처리에 실패했습니다.') } }
  const navItems: ConsoleNavItem<'dashboard' | 'api-keys' | 'analytics' | 'tax'>[] = [{ id: 'dashboard', label: '대시보드' }, { id: 'api-keys', label: 'API 키 관리' }, { id: 'analytics', label: '사용량 분석' }, { id: 'tax', label: '세무 처리' }]
  const totalClicks = stats.reduce((sum, stat) => sum + stat.clickCount, 0)
  return <ConsoleShell title="Hana OmniLens 관리자 백오피스" user={session.user} navItems={navItems} activeNav={activeTab} onNavigate={setActiveTab} onHome={onHome} onPassword={onPassword} onSignOut={onSignOut}>{message && <p className="notice">{message}</p>}{activeTab === 'dashboard' && <><div className="console-hero"><div><p className="eyebrow">ADMIN OVERVIEW</p><h2>운영 현황</h2><p>API 키, 사용량, 세무 신청을 탭별로 관리합니다.</p></div><button className="secondary" onClick={() => void refresh()}>새로고침</button></div><div className="admin-summary"><Summary label="API 키 신청" value={applications.length}/><Summary label="세무 처리 신청" value={cases.length}/><Summary label="고유어 설명 클릭" value={totalClicks}/></div><div className="admin-quick-links"><button onClick={() => setActiveTab('api-keys')}><span>API KEY</span><b>API 키 신청 처리</b><i>→</i></button><button onClick={() => setActiveTab('analytics')}><span>ANALYTICS</span><b>사용량 통계 확인</b><i>→</i></button><button onClick={() => setActiveTab('tax')}><span>TAX</span><b>경정청구서 처리</b><i>→</i></button></div></>}{activeTab === 'api-keys' && <section className="admin-panel tab-panel"><div className="panel-title"><div><p className="eyebrow">API KEY MANAGEMENT</p><h1>API 키 관리</h1><p>파트너 신청을 검토하고 승인합니다.</p></div></div><ApplicationTable applications={applications} admin onApprove={approve}/></section>}{activeTab === 'analytics' && <section className="tab-panel"><div className="page-heading"><p className="eyebrow">USAGE ANALYTICS</p><h1>사용량 분석</h1><p>금융 고유어 설명 API의 클릭과 캐시 사용량을 확인합니다.</p></div><div className="admin-summary"><Summary label="전체 설명 클릭" value={totalClicks}/><Summary label="캐시 적중" value={stats.reduce((sum, stat) => sum + stat.cacheHitCount, 0)}/><Summary label="집계 용어" value={stats.length}/></div><Analytics stats={stats}/></section>}{activeTab === 'tax' && <section className="tab-panel"><div className="page-heading"><p className="eyebrow">TAX OPERATIONS</p><h1>세무 처리</h1><p>AI 검증 서류로 경정청구서를 작성하고 승인합니다.</p></div><TaxAdmin cases={cases} token={session.accessToken} onChanged={refresh}/></section>}</ConsoleShell>
}
...
function TaxAdmin({ cases, token, onChanged }: { cases: TaxCase[]; token: string; onChanged: () => Promise<void> }) {
  ...
  const chooseTemplate = async (file?: File) => { if (!file) return; if (file.type !== 'application/pdf' || file.size > 10 * 1024 * 1024) { setMessage('PDF 양식은 10MB 이하만 첨부할 수 있습니다.'); return } try { const value = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result).split(',')[1] ?? ''); reader.onerror = () => reject(reader.error); reader.readAsDataURL(file) }); setTemplate(value); setMessage('PDF 양식을 불러왔습니다.') } catch { setMessage('PDF 양식을 읽지 못했습니다.') } }
  ...
  const submit = async () => { if (!selected || !window.confirm('현재 편집 값으로 경정청구서를 확정하고 국세청 제출·회원 승인 처리하시겠습니까?')) return; try { await api(`/api/v1/portal/admin/tax/refund-cases/${selected.caseId}/approve`, { method: 'POST', body: JSON.stringify({ fields, templateBase64: template }) }, token); await onChanged(); setSelected(null); setMessage('경정청구서를 저장하고 회원 환급 상태를 승인으로 변경했습니다.') } catch (error) { setMessage(error instanceof Error ? error.message : '승인 처리에 실패했습니다.') } }
  return <section className="admin-panel"><p className="eyebrow">TAX OPERATIONS</p><h2>세무 서류 관리</h2>{cases.length === 0 ? <p className="empty">동기화된 신청 건이 없습니다.</p> : cases.map((item) => <button className="case-row" key={item.caseId} onClick={() => void open(item)}><div><b>{item.caseId}</b><span>{item.taxYear} · {item.treatyCountry} · USD {item.estimatedRefundUsd}</span></div><strong>{item.status === 'REFUND_APPROVED' ? '승인' : item.correctionRequestStatus === 'PREPARED' ? '작성 완료' : '미작성'}</strong></button>)}{selected && <div className="tax-editor"><p className="eyebrow">경정청구서 편집기</p><h3>{selected.caseId}</h3><p className="muted">검증 완료 서류: {selected.verifiedDocuments.map((document) => document.fileName).join(' · ')}</p><label>경정청구서 PDF 양식<input type="file" accept="application/pdf" onChange={(event) => void chooseTemplate(event.target.files?.[0])}/><small>양식을 첨부하려면 편집 항목과 같은 이름의 PDF 폼 필드가 필요합니다.</small></label>{Object.entries(fields).map(([key, value]) => <label key={key}>{key}<input value={value} onChange={(event) => setFields({ ...fields, [key]: event.target.value })}/></label>)}<div className="editor-actions"><button onClick={() => void download()}>PDF 다운로드</button><button className="primary" disabled={selected.status === 'REFUND_APPROVED'} onClick={() => void submit()}>국세청 제출 및 승인</button></div></div>}{message && <p className="notice">{message}</p>}</section>
}
