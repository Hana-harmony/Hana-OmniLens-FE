...
export function ProductStory({ locale, title, intro }: { locale: Locale; title: string; intro: string }) {
  ...
  const stories: StoryScene[] = locale === 'ko' ? [
    ...
    { chapter: '핵심 기능 03 · STEP 3', title: '경정청구 백오피스', body: '하나증권 백오피스에서 외국 금융사 고객별 경정청구 진행 현황을 실시간으로 모니터링하고, 신청서 자동 작성과 검토를 거쳐 국세청에 원클릭으로 제출할 수 있도록 지원합니다.', signal: '실시간 진행 현황 · 신청서 자동 작성 · 국세청 원클릭 제출', kind: 'tax-backoffice', phones: [asset('tax-backoffice', '제한세율 적용신청 처리 백오피스 화면', 'gif'), asset('tax-backoffice-submit', '국세청 원클릭 제출 화면')] },
  ] : [
    ...
    { chapter: 'CORE 03 · STEP 3', title: 'Tax-correction back office', body: 'The Hana Securities back office monitors correction-claim progress by overseas financial-institution client in real time, automatically prepares and reviews applications, and supports one-click submission to the National Tax Service.', signal: 'Live status · automatic application · one-click submission', kind: 'tax-backoffice', phones: [asset('tax-backoffice', 'Tax-application processing back-office screen', 'gif'), asset('tax-backoffice-submit', 'National Tax Service submission screen')] },
  ]
