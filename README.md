...
- 모델 소개 화면에 공식명 Hana Montana AI(KF-DeBERTa + K-FNSPID), 뉴스·공시 출처별 모델 계약과 확정 데이터 규모를 표시한다. 잠금 후 확증 1차 가중 Macro-F1은 뉴스 0.5530 대 원본 금융 특화 기준모델 0.4937, 공시 0.6024 대 0.6146이며 후보는 미승격으로 표시한다.

## 뉴스·공시 AI 표시 기준

- 기존 모델 성능 그래프와 확정값은 유지하며, 감성 비교 영역만 잠금 후 확증 결과로 갱신한다.
- 이벤트·종목 분류의 실제 뉴스 이벤트 Macro-F1 `0.9221`, 공시 의미 중요도의 기존 모델 `0.8355`·모델 단독 `0.9470`·운영 Gold `0.9962`를 계속 표시한다.
- 감성 확증 가중 Macro-F1은 뉴스 `0.5530` 대 원본 금융 특화 기준모델 `0.4937`, 공시 `0.6024` 대 `0.6146`이며, 두 출처를 함께 개선하지 못해 후보를 미승격으로 표시한다.
...
`npm test`는 기존 사용자 흐름과 함께 모델 공식명, 확정 성능값 보존, 감성 확증 수치와 한·영 표시를 검증한다. `npm run build`는 TypeScript 프로젝트 검사와 Vite 운영 빌드를 모두 수행한다.
