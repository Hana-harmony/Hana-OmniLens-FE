...
- 캡처 이미지 없이 실제 웹 UI로 구성한 8단계 핵심 기능 소개와 데스크톱 고정형 스크롤 전환·모바일 순차형 반응형 화면
...
`npm test`는 새로고침 가능한 콘솔 탭 URL 계약, Omni-Connect 브랜드, 캡처 이미지에 의존하지 않는 핵심 기능 UI 계약을 검증한다. `npm run build`는 TypeScript 프로젝트 검사와 Vite 운영 빌드를 모두 수행한다.
