...
- `src/modelCatalog.ts`를 모델 공식명, 한·영 소개, 출처, 검증 상태, 확정 데이터 규모의 단일 정본으로 사용한다.
- 감성·의미 중요도·시장영향은 NEWS와 DISCLOSURE 카드를 따로 두며 서로의 표본·지표를 합쳐 표시하지 않는다.
- 동일한 평가 세트에서 비교 모델과 함께 측정하지 않은 성능은 사용자 화면에 수치로 표시하지 않는다. 측정 완료 전에는 `성능 측정 중`으로 표시한다.
...
`npm test`는 기존 사용자 흐름과 함께 모델 카탈로그의 공식명, 출처 분리, value/display·한/영 숫자 일치, 성능 측정 상태를 검증한다. `npm run build`는 TypeScript 프로젝트 검사와 Vite 운영 빌드를 모두 수행한다.
